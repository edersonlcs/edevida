...
const responseSchema = {
  ...
  schema: {
    ...
    properties: {
      ...
      fat_good_g: { type: "number", minimum: 0 },
      fat_bad_g: { type: "number", minimum: 0 },
...
function buildSystemPrompt() {
  ...
  return [
    ...
    "Informe estimativas nutricionais no total da refeicao e por alimento: estimated_calories, protein_g, carbs_g, fat_g, fat_good_g, fat_bad_g, sodium_mg, sugar_g.",
    "fat_good_g representa gordura insaturada/protetora; fat_bad_g representa gordura saturada/trans potencialmente prejudicial.",
    "Se nao houver dado preciso, estime de forma conservadora usando composicao tipica dos alimentos.",
...
function formatNutritionReply(analysis) {
  ...
  return [
    ...
    `Gordura boa estimada: ${analysis.fat_good_g ?? 0} g`,
    `Gordura ruim estimada: ${analysis.fat_bad_g ?? 0} g`,
    `Sodio estimado: ${analysis.sodium_mg ?? 0} mg`,
    `Acucar estimado: ${analysis.sugar_g ?? 0} g`,
    itemsPreview ? `Itens:\n${itemsPreview}` : "",
  ].join("\n");
}
